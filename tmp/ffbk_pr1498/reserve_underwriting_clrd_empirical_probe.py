...
- seven insurer groups with a complete 1988-1997 panel and positive direct
  earned premium;
- observations are insurer x calendar-year pairs for 1989-1997.

For calendar year t:
  UW_t = first-reported incurred loss for AY=t / direct earned premium AY=t.
  LEGACY_t = sum_{AY<t}(Incurred[AY,t]-Incurred[AY,t-1])
             / sum_{AY<t} Incurred[AY,t-1].

A premium-normalized legacy sensitivity and an unscaled emergence sensitivity
are reported. Two-way demeaning removes additive insurer and calendar-year
means from both variables before a residual correlation is computed.
...
def load_rows(path: Path, lob: str, grcodes: Sequence[int]) -> Tuple[Dict[RowKey, dict], Dict[int, str]]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        required = {"GRCODE", "GRNAME", "AccidentYear", "DevelopmentYear", "EarnedPremDIR", "LOB", incurred_col}
...
def build_panel(
    ...
    for grcode in grcodes:
        for year in range(start_ay + 1, end_ay + 1):
            ...
            if current is None or current["prem_dir"] <= 0:
                continue
            uw = current["incurred"] / current["prem_dir"]

            legacy_delta = 0.0
            legacy_open_incurred = 0.0
            legacy_open_premium = 0.0
            n_legacy = 0
            complete = True
            for ay in range(start_ay, year):
                ...
                if prev is None or now is None or base is None:
                    complete = False
                    break
                legacy_delta += now["incurred"] - prev["incurred"]
                legacy_open_incurred += prev["incurred"]
                legacy_open_premium += base["prem_dir"]
                n_legacy += 1

            if not complete or n_legacy == 0:
                continue
            if legacy_open_incurred <= 0 or legacy_open_premium <= 0:
                continue
            ...
            panel.append(
                {
                    ...
                    "legacy_premium_norm": legacy_delta / legacy_open_premium,
                    "legacy_level": legacy_delta,
                    "legacy_open_incurred": legacy_open_incurred,
                    "legacy_open_premium": legacy_open_premium,
                }
            )
    return panel
...
def correlation_block(panel: Sequence[dict], legacy_field: str) -> dict:
    uw = [float(row["uw"]) for row in panel]
    legacy = [float(row[legacy_field]) for row in panel]
    uw_resid = two_way_demean(panel, "uw")
...
def run(path: Path, lob: str, grcodes: Sequence[int], start_ay: int, end_ay: int) -> dict:
    ...
    return {
        "design": {
            ...
            "underwriting_proxy": "first-development incurred / direct earned premium",
            "legacy_proxy": "older-AY calendar-year incurred emergence / prior-calendar-year older-AY incurred",
            "conditioning": "two-way additive demeaning by insurer and calendar year",
            "classification": "exploratory descriptive empirical challenge; not economic CDR; not tail calibration",
        },
        "panel": {
            "n": len(panel),
        ...
        "sensitivities": {
            "legacy_premium_normalized": correlation_block(panel, "legacy_premium_norm"),
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="fail unless the default pilot is a balanced 7x9 panel")
    ...
    if args.check:
        ...
        if expected_default:
            if result["panel"]["n"] != 63 or result["panel"]["companies"] != 7 or len(result["panel"]["years"]) != 9:
                raise AssertionError(f"default pilot is not the expected balanced 7x9 panel: {result['panel']}")
