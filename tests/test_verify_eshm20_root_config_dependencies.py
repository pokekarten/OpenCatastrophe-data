...
SYNTHETIC_PATHS = frozenset(
    {
        module.PREFIX + "sites.csv",
        module.PREFIX + "source.xml",
        module.PREFIX + "gmpe.xml",
    }
)


class VerifiedEshm20RootConfigDependenciesTests(unittest.TestCase):
    def frozen_to(self, payload: bytes, *, inventory: frozenset[str] = SYNTHETIC_PATHS):
    ...
    def test_verified_bytes_are_parsed_deterministically(self) -> None:
        with self.frozen_to(SYNTHETIC, inventory=frozenset((*SYNTHETIC_PATHS, *range(59)))):
        ...
        self.assertEqual(
            ...
            [module.PREFIX + "gmpe.xml", module.PREFIX + "sites.csv", module.PREFIX + "source.xml"],
        )
    ...
    def test_public_parser_bridge_has_no_receipt_identity_override(self) -> None:
        self.assertEqual(list(inspect.signature(module.extract_verified_root_dependencies).parameters), ["payload"])
    ...
    def test_digest_mismatch_blocks_before_parser_without_observed_digest(self) -> None:
        ...
        with mock.patch.object(module, "extract_openquake_config_references") as parser:
            with self.assertRaisesRegex(module.VerifiedRootConfigError, "SHA-256 mismatch") as caught:
    ...
    def test_parser_failure_is_rewrapped_without_payload(self) -> None:
        ...
        with self.frozen_to(payload):
            with self.assertRaisesRegex(module.VerifiedRootConfigError, "verified root dependency parse failed") as caught:
                module.extract_verified_root_dependencies(payload)
        self.assertNotIn(payload.decode("utf-8"), str(caught.exception))

    def test_in_repository_dependency_absent_from_frozen_inventory_fails_closed(self) -> None:
        payload = b"[general]\nsite_model_file = syntactically-valid-but-absent.csv\n"
        inventory = frozenset({module.REPOSITORY_PATH, *range(61)})
        with self.frozen_to(payload, inventory=inventory):
            with self.assertRaisesRegex(module.VerifiedRootConfigError, "absent from frozen inventory") as caught:
                module.extract_verified_root_dependencies(payload)
        self.assertNotIn("syntactically-valid-but-absent.csv", str(caught.exception))

    def test_frozen_receipt_and_inventory_identity_is_exact(self) -> None:
        self.assertEqual(module.EXPECTED_BYTE_COUNT, 2719)
        self.assertEqual(module.EXPECTED_SHA256, "f1f4dabc48e1b8a478dbdb96b01c8f58cc68c98abd6f9004671c5fba9eb7e714")
        self.assertEqual(module.SOURCE_ISSUE, 281)
        self.assertEqual(module.PROJECT_ID, 197)
        self.assertEqual(module.COMMIT_SHA, "fbd334de68f85d72669f73fc5a314a113db67317")
        ...
        self.assertIn(module.PREFIX + "eshm20_site_model_v06d.csv", module.FROZEN_INVENTORY_PATHS)
        self.assertIn(module.PREFIX + "gmpe_complete_logic_tree_5br.xml", module.FROZEN_INVENTORY_PATHS)
        self.assertIn(module.PREFIX + "source_model_logic_tree_eshm20_model_v12e.xml", module.FROZEN_INVENTORY_PATHS)
