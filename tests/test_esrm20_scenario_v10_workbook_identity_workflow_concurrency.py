...
WORKFLOW = Path(".github/workflows/esrm20-scenario-v10-workbook-identity.yml")
REQUEST_MARKER = "<!-- oc-eq1-esrm20-scenario-v10-workbook-identity-request-v1 -->"


class ScenarioWorkbookIdentityWorkflowConcurrencyTests(unittest.TestCase):
    def test_trusted_requests_serialize_but_comment_noise_is_isolated(self) -> None:
        ...
        self.assertNotIn(
            "group: esrm20-scenario-v10-workbook-identity-${{ github.repository }}\n",
            concurrency,
        )

    def test_execution_job_keeps_closed_request_fence(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        execute = text.split("execute-workbook-identity:", 1)[1].split(
            "publish-workbook-identity:", 1
        )[0]
