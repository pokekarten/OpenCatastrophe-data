...
class SiteModelProfileTerminalSizeTests(unittest.TestCase):
    def _assert_rejected_before_json(self, body: str, message: str) -> None:
        with mock.patch.object(
            action.json,
            "loads",
            side_effect=AssertionError(
                "JSON parser must not receive unbounded terminal text"
            ),
        ) as loads:
            with self.assertRaisesRegex(action.SiteModelProfileActionError, message):
