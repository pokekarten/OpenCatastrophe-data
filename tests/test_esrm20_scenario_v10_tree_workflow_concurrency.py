...
WORKFLOW = Path(".github/workflows/esrm20-scenario-v10-tree.yml")
REQUEST_MARKER = "<!-- oc-eq1-esrm20-scenario-v10-tree-request-v1 -->"


class ScenarioTreeWorkflowConcurrencyTests(unittest.TestCase):
    def test_trusted_requests_serialize_but_comment_noise_is_isolated(self) -> None:
        ...
        self.assertNotIn(
            "group: esrm20-scenario-v10-tree-${{ github.repository }}\n",
            concurrency,
        )

    def test_execution_job_keeps_closed_request_fence(self) -> None:
        text = WORKFLOW.read_text(encoding="utf-8")
        execute = text.split("execute-scenario-tree:", 1)[1].split(
            "publish-scenario-tree:", 1
        )[0]
        self.assertIn("github.event.issue.number == 285", execute)
        self.assertIn(
            "github.event.comment.user.login == github.event.repository.owner.login",
            execute,
        )
