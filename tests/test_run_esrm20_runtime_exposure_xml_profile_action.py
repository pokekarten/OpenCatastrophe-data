...
def evidence():
    return {
        ...
        "profile": {
            ...
            "asset_references": ["Exposure_Kosovo.csv"],
            "cost_types": [],
            "area": None,
            "occupancy_periods": [],
            "tag_names": [],
...
class ActionTests(unittest.TestCase):
    ...
    def test_failures_are_bounded(self):
        ...
        for exc, failure_class, failure_code in failures:
            with (
                self.subTest(failure_class=failure_class),
                mock.patch.object(
                    subject, "profile_runtime_exposure_xml", side_effect=exc
                ),
            ):
                result = subject.run_profile(execution_sha=SHA)
    ...
    def test_unknown_xml_error_cannot_leak_exception_text(self):
        ...
        self.assertEqual(result["failure_class"], "xml_profile_failure")
        self.assertEqual(
            result["failure_code"], "unclassified_xml_profile_failure"
        )
    ...
    def test_duplicate_json_keys_fail_closed(self):
        ...
        with self.assertRaisesRegex(
            subject.RuntimeExposureXmlProfileActionError, "duplicate JSON key"
        ):
