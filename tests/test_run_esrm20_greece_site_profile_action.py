...
def _payload():
    return {
        ...
        "profile": {
            ...
            "tag_counts": [
                {
                    "name": {"namespace": "urn:test", "local_name": "nrml"},
                    "count": 2,
                }
            ],
            "namespace_counts": [],
...
def _terminal_body(execution_sha: str) -> str:
    result = subject._run_greece_site_profile(
...
class GreeceSiteProfileActionTests(unittest.TestCase):
    def test_request_is_exactly_bound_to_issue_dataset_action_and_execution_sha(self):
        ...
        for bad in (
            ...
            with self.subTest(bad=bad):
                with self.assertRaises(subject.GreeceSiteProfileActionError):
    ...
    def test_duplicate_keys_nonfinite_json_and_unsafe_requester_fail_closed(self):
        duplicate = (
            ...
            + '","action":"'
            + subject.ACTION
            + '","issue":661,"target_sha":"'
            + SHA
            + '","dataset_id":"'
            + subject.DATASET_ID
            + '","requester":"x","requester":"y"}'
        )
        with self.assertRaisesRegex(subject.GreeceSiteProfileActionError, "duplicate"):
            subject.validate_request(
                duplicate,
                expected_issue=661,
                execution_sha=SHA,
        ...
        nonfinite = _request(requester="unit-test").replace(
            '"requester":"unit-test"',
            '"requester":NaN',
        )
        with self.assertRaises(subject.GreeceSiteProfileActionError):
        ...
        with self.assertRaisesRegex(
            subject.GreeceSiteProfileActionError,
            "requester",
        ):
            subject.validate_request(
    ...
    def test_pass_preserves_exact_identity_and_all_authority_ceilings(self):
        result = subject._run_greece_site_profile(
            execution_sha=SHA,
            acquirer=_payload,
        )
        self.assertEqual(result["status"], "pass")
        self.assertEqual(result["source_issue"], 661)
        self.assertEqual(result["content_issue"], 285)
        self.assertEqual(
            result["site_identity"]["repository_path"],
            "Vs30/Site_model_Greece.xml",
        )
        self.assertEqual(result["site_identity"]["receipt_comment_id"], 5_388_640_521)
    ...
    def test_acquisition_failure_is_bounded(self):
        def fail():
            raise subject.GreeceSiteProfileAcquisitionError("provider unavailable")
    ...
    def test_profile_failure_is_distinct_and_bounded(self):
        def fail():
            raise subject.GreeceSiteProfileContentError("exact bytes failed parser")
    ...
    def test_contract_failure_is_not_sanitized_as_provider_failure(self):
        def fail():
            raise subject.GreeceSiteProfileContractError("trusted contract drift")

        with self.assertRaises(subject.GreeceSiteProfileContractError):
            subject._run_greece_site_profile(
                execution_sha=SHA,
                acquirer=fail,
            )
    ...
    def test_dedup_trusts_only_actions_bot_and_exact_execution(self):
        ...
        with mock.patch.object(
            ...
            self.assertTrue(
                subject.has_terminal_greece_site_profile_result(
