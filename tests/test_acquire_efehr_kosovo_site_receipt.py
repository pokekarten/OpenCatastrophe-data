...
from scripts.acquire_efehr_kosovo_site_receipt import (
    COMMIT_SHA,
    DATASET_ID,
...
class KosovoSiteReceiptTests(unittest.TestCase):
    def test_fixed_worker_hashes_only_the_immutable_site_object(self) -> None:
        ...
        receipt = acquire_kosovo_site_receipt(
            opener=opener,
    ...
    def test_worker_has_no_caller_controlled_provider_target_surface(self) -> None:
        parameters = set(inspect.signature(acquire_kosovo_site_receipt).parameters)
        self.assertEqual(parameters, {"opener", "now", "monotonic"})
        self.assertEqual(SOURCE_ISSUE, 284)
        ...
        self.assertEqual(REPOSITORY_PATH, "Vs30/Site_model_Kosovo.xml")
    ...
    def test_declared_oversize_and_empty_payload_fail_closed(self) -> None:
        ...
        oversized = FakeResponse(
            b"x",
            url,
            headers={"Content-Length": str(MAX_SITE_MODEL_BYTES + 1)},
        )
