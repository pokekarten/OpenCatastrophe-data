...
class Esrm20EventHazardReceiptTests(unittest.TestCase):
    def test_both_fixed_workers_hash_only_their_immutable_configs(self) -> None:
        cases = (
            ...
            with self.subTest(path=path):
                ...
                def opener(request, timeout):
                    ...
                    return FakeResponse(
                        payload,
                        url,
                        headers={"Content-Length": str(len(payload)), "Content-Type": "text/plain", "ETag": '"synthetic"'},
                    )
    ...
    def test_public_workers_expose_no_group_or_provider_target_selector(self) -> None:
        for worker in (acquire_event_hazard_group1_receipt, acquire_event_hazard_group2_receipt):
    ...
    def test_transport_failure_is_sanitized(self) -> None:
        ...
        with self.assertRaisesRegex(EfehrAcquisitionError, r"artifact retrieval failed: OSError$"):
            acquire_event_hazard_group2_receipt(opener=opener, now=lambda: RETRIEVED_AT, monotonic=lambda: 0.0)
