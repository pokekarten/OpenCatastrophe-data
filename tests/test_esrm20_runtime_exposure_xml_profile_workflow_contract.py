...
class WorkflowContractTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.text = Path(
            ".github/workflows/esrm20-runtime-exposure-xml-profile.yml"
    ...
    def test_result_v5_publisher_accepts_only_bounded_nrml_namespaces(self):
        publisher = self.text.split("publish-profile:", 1)[1]
        self.assertIn(
        ...
        self.assertIn(
            '.schema_version == "oc-esrm20-runtime-exposure-xml-profile-result-v5"',
            publisher,
        )
        self.assertIn(
            '.profile.nrml_namespace == "http://openquake.org/xmlns/nrml/0.4"',
            publisher,
        )
        self.assertIn(
            '.profile.nrml_namespace == "http://openquake.org/xmlns/nrml/0.5"',
            publisher,
        )
        self.assertNotIn('"nrml_root_namespace_legacy_04"', publisher)
        self.assertNotIn("result-v4", publisher)
