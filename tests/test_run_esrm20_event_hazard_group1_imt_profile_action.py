...
from scripts import run_esrm20_event_hazard_group1_imt_profile_action as subject
...
class Group1ImtProfileActionTests(unittest.TestCase):
    ...
    def test_rejects_request_bound_to_other_execution_sha(self) -> None:
        with self.assertRaisesRegex(subject.Group1ImtProfileActionError, "target_sha drifted"):
            subject.validate_request(
                _request(target_sha="2" * 40),
                expected_issue=subject.CONTROL_ISSUE,
                execution_sha=EXECUTION_SHA,
            )
    ...
    def test_pass_result_preserves_false_authority_ceiling(self) -> None:
        ...
        self.assertEqual(result["profile"]["imt_names"], ["PGA", "SA(0.3)", "SA(0.6)", "SA(1.0)"])
    ...
    def test_profile_cannot_widen_component_or_unit_authority(self) -> None:
        ...
        with self.assertRaisesRegex(
            subject.Group1ImtProfileActionError,
            "drifted at component_semantics_verified",
        ):
    ...
    def test_terminal_parser_accepts_only_exact_execution_result(self) -> None:
        ...
        self.assertFalse(subject._parse_trusted_terminal_result(body, execution_sha="2" * 40))
