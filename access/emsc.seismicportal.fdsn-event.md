...
# Access contract: `emsc.seismicportal.fdsn-event.json`
...
```json
{
  ...
  "access_id": "emsc.seismicportal.fdsn-event",
  "source_ids": [
  ...
  "interface_type": "fdsn",
  "status": "probe_ready",
  "documentation_url": "https://www.seismicportal.eu/fdsn-wsevent.html",
  "service_root": "https://www.seismicportal.eu",
  "api_version": "FDSN Event service /1; OpenAPI 2.2",
  "access_scope": [
    "catalogue",
    "sample"
  ],
  ...
  "request_contract": {
    ...
    "parameter_rules": "The initial probe is repository-constructed only: format=json, limit=1, orderby=time, nodata=204, includeallorigins=false and includearrivals=false. Callers cannot supply a host, arbitrary path, headers, event identifier, catalogue, contributor, geographic filter, time range, pagination offset or unrestricted query parameters. Any later parameterized scientific query requires a separately reviewed bounded operation rather than widening this probe contract."
  },
  "response_contract": {
  ...
  "operational_constraints": {
    ...
    "rate_limit_notes": "No repository-specific request-rate assumption is made. The provider documents a maximum query limit of 20000 events; this contract intentionally fixes the probe limit to one event and must not be used for bulk harvesting.",
    "mutability_notes": "The EMSC event catalogue is operational and mutable. Every future probe receipt must bind retrieval UTC, exact trusted execution code, normalized request identity, response byte count and SHA-256; a later scientific run must separately freeze the event/product identity it actually uses."
  },
  "rights_and_policy": {
    "dataset_rights_status": "verified",
    "api_terms_status": "separate_reviewed",
    "terms_url": "https://www.seismicportal.eu/terms.html",
