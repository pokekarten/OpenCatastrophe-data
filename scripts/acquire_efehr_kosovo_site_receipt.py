...
OPERATION_ID = "esrm20-kosovo-site-model-v1"
SOURCE_ISSUE = 284
...
MAX_SITE_MODEL_BYTES = MAX_FILE_BYTES


def acquire_kosovo_site_receipt(
    *,
    opener: Any | None = None,
    now: Any = utc_now,
    monotonic: Any = time.monotonic,
) -> dict[str, Any]:
    """Fetch only the frozen Kosovo site XML and return its byte receipt.

    Provider bytes are streamed into the receipt primitive and are not returned
    or persisted by this worker. The immutable commit is the execution identity;
    no mutable tag or branch is resolved here.
    """
    ...
    try:
        with open_response(request, timeout=_remaining(deadline, monotonic)) as response:
            ...
            _declared_length(response, MAX_SITE_MODEL_BYTES)
            retrieved_at = now()
            try:
                core_receipt = receipt_from_stream(
                    ...
                    max_bytes=MAX_SITE_MODEL_BYTES,
                )
            except EfehrReceiptError as exc:
