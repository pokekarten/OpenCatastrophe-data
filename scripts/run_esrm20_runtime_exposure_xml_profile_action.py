...
_REQUEST_FIELDS = {
    "schema_version", "action", "issue", "target_sha", "dataset_id", "receipt_sha256", "requester"
}
_RESULT_FIELDS = {
...
def _strict_loads(text: str, *, label: str) -> Any:
    try:
        return json.loads(text, object_pairs_hook=_pairs, parse_constant=_reject_constant, parse_float=_parse_float)
...
def validate_request(body: object, *, expected_issue: int, execution_sha: str) -> dict[str, Any]:
    ...
    exact = (
        ("schema_version", REQUEST_SCHEMA_VERSION), ("action", ACTION), ("issue", SOURCE_ISSUE),
        ("target_sha", execution_sha), ("dataset_id", DATASET_ID), ("receipt_sha256", EXPECTED_SHA256),
    )
...
def _identity() -> dict[str, Any]:
    return {
        "project_id": PROJECT_ID, "project_path": PROJECT_PATH, "commit_sha": COMMIT_SHA,
        "repository_path": REPOSITORY_PATH, "receipt_byte_count": EXPECTED_BYTE_COUNT,
...
def _validate_profile(profile: object) -> dict[str, Any]:
    ...
    if type(model) is not dict or set(model) != {"id", "category", "taxonomy_source", "description"}:
    ...
    if type(assets) is not list or not assets or any(type(item) is not str or not item for item in assets):
        raise RuntimeExposureXmlProfileActionError("invalid asset references")
    for key in ("cost_types", "occupancy_periods", "tag_names", "exposure_fields", "structural_value_inputs"):
    ...
    if type(profile.get("structural_cost_type_declared")) is not bool:
        raise RuntimeExposureXmlProfileActionError("invalid structural declaration flag")
    return profile
...
def _validate_terminal_result(result: object) -> str:
    ...
    exact = (
        ("schema_version", RESULT_SCHEMA_VERSION), ("action", ACTION), ("source_issue", SOURCE_ISSUE),
        ("dataset_id", DATASET_ID), ("target_sha", execution_sha), ("runtime_exposure_identity", _identity()),
        ("exact_kosovo_exposure_selected", False), ("value_structural_wiring_verified", False),
        ("external_bytes_persisted", False), ("publication_authorized", False), ("model_use_authorized", False),
    )
    for field, expected in exact:
