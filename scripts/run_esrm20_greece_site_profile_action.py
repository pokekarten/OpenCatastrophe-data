...
"""Closed trusted-main action for the exact ESRM20 Greece site structure profile."""
...
try:
    from scripts.acquire_efehr_greece_site_profile import (
        GreeceSiteProfileAcquisitionError,
        GreeceSiteProfileContentError,
        GreeceSiteProfileContractError,
        _validate_profile_result,
        acquire_and_profile_greece_site,
    )
    from scripts.prepare_agent_action_result import LedgerError, fetch_repository_comments
except ModuleNotFoundError:  # pragma: no cover - direct script execution path
    from acquire_efehr_greece_site_profile import (
        GreeceSiteProfileAcquisitionError,
        GreeceSiteProfileContentError,
        GreeceSiteProfileContractError,
        _validate_profile_result,
...
REQUEST_MARKER = "<!-- oc-eq1-esrm20-greece-site-profile-request-v1 -->"
RESULT_MARKER = "<!-- oc-eq1-esrm20-greece-site-profile-result-v1 -->"
REQUEST_SCHEMA_VERSION = "oc-esrm20-greece-site-profile-request-v1"
RESULT_SCHEMA_VERSION = "oc-esrm20-greece-site-profile-result-v1"
ACTION = "esrm20_greece_site_model_structure_profile"
CONTROL_ISSUE = 661
SOURCE_ISSUE = 285
...
class GreeceSiteProfileActionError(RuntimeError):
    """Fail-closed trusted Greece site-profile action error."""
...
def _pairs(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in obj:
            raise GreeceSiteProfileActionError("duplicate JSON key")
        obj[key] = value
    return obj


def _reject_constant(value: str) -> Any:
    raise GreeceSiteProfileActionError(f"non-finite JSON constant: {value}")
...
def validate_request(
    ...
    if type(expected_issue) is not int or expected_issue != CONTROL_ISSUE:
        raise GreeceSiteProfileActionError("wrong runtime issue")
    if type(execution_sha) is not str or not _SHA_RE.fullmatch(execution_sha):
        raise GreeceSiteProfileActionError("invalid execution SHA")
    if type(body) is not str or body.count(REQUEST_MARKER) != 1:
        raise GreeceSiteProfileActionError("invalid Greece site-profile request marker")
    before, after = body.split(REQUEST_MARKER, 1)
    if before.strip() or not after.strip():
        raise GreeceSiteProfileActionError(
            "Greece site-profile request envelope is not canonical"
        )
    try:
        request = json.loads(
            after.strip(),
    ...
    except GreeceSiteProfileActionError:
        raise
    except (json.JSONDecodeError, TypeError, ValueError) as exc:
        raise GreeceSiteProfileActionError(
            "invalid Greece site-profile request JSON"
        ) from exc
    if type(request) is not dict or set(request) != _REQUEST_FIELDS:
        raise GreeceSiteProfileActionError("Greece site-profile request fields drifted")
    exact = (
    ...
    for field, expected in exact:
        ...
        if type(observed) is not type(expected) or observed != expected:
            raise GreeceSiteProfileActionError(
                f"Greece site-profile request {field} drifted"
            )
    ...
    if (
        ...
        raise GreeceSiteProfileActionError("invalid requester identity")
    return request
...
def _validate_profile_payload(payload: object) -> dict[str, Any]:
    ...
    except GreeceSiteProfileContractError as exc:
        raise GreeceSiteProfileActionError(
            "Greece site-profile payload violates reviewed worker contract"
        ) from exc
...
def _validate_terminal_result(
    ...
    if type(result) is not dict or set(result) != expected_fields:
        raise GreeceSiteProfileActionError(
            "trusted Greece site-profile result fields drifted"
        )
    for field, expected in base.items():
        observed = result.get(field)
        if type(observed) is not type(expected) or observed != expected:
            raise GreeceSiteProfileActionError(
                f"trusted Greece site-profile result drifted at {field}"
            )
    status = result.get("status")
    if status == "pass":
        if result.get("failure_class") is not None:
            raise GreeceSiteProfileActionError(
                "Greece site-profile PASS cannot carry failure_class"
            )
    ...
    if status == "blocked":
        if (
            ...
            raise GreeceSiteProfileActionError(
                "blocked Greece site-profile result is not safely bounded"
            )
        return result
    raise GreeceSiteProfileActionError(
        "trusted Greece site-profile result has non-terminal status"
    )
...
def _parse_trusted_terminal_result(body: object, *, execution_sha: str) -> bool:
    ...
    if body.count(RESULT_MARKER) != 1:
        raise GreeceSiteProfileActionError(
            "trusted Greece site-profile result marker is malformed"
        )
    before, after = body.split(RESULT_MARKER, 1)
    if before.strip() or not after.strip():
        raise GreeceSiteProfileActionError(
            "trusted Greece site-profile result envelope is malformed"
        )
    try:
        result = json.loads(
            after.strip(),
        ...
        raise GreeceSiteProfileActionError(
            "trusted Greece site-profile result JSON is malformed"
        ) from exc
...
def has_terminal_greece_site_profile_result(
    *,
    ...
    """Fail closed unless the complete bounded Issue #661 ledger is known."""
    ...
    try:
        ...
        raise GreeceSiteProfileActionError(
            "Greece site-profile result ledger is incomplete"
        ) from exc
    for comment in comments:
        if type(comment) is not dict:
            raise GreeceSiteProfileActionError(
                "Greece site-profile ledger contains a non-object comment"
            )
...
def _run_greece_site_profile(
    *,
    ...
    try:
        payload = acquirer()
    except GreeceSiteProfileAcquisitionError:
        result.update(
            {
                "status": "blocked",
    ...
    except GreeceSiteProfileContentError:
        result.update(
            {
                "status": "blocked",
...
def run_greece_site_profile(*, execution_sha: str) -> dict[str, Any]:
    return _run_greece_site_profile(
        execution_sha=execution_sha,
        acquirer=acquire_and_profile_greece_site,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_greece_site_profile(execution_sha=args.execution_sha)
