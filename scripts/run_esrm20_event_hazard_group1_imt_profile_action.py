...
"""Trusted-main bounded IMT-name profile for the exact ESRM20 Group1 root."""
...
REQUEST_MARKER = "<!-- oc-eq1-esrm20-group1-imt-profile-request-v1 -->"
RESULT_MARKER = "<!-- oc-eq1-esrm20-group1-imt-profile-result-v1 -->"
REQUEST_SCHEMA_VERSION = "oc-esrm20-event-hazard-group1-imt-profile-request-v1"
RESULT_SCHEMA_VERSION = "oc-esrm20-event-hazard-group1-imt-profile-result-v1"
ACTION = "esrm20_event_hazard_group1_imt_profile"
CONTROL_ISSUE = 281
...
_CANONICAL_ACQUIRER = _acquisition.acquire_event_hazard_group1_imt_profile


class Group1ImtProfileActionError(RuntimeError):
    """Fail-closed trusted Group1 IMT profile error."""
...
def _pairs(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise Group1ImtProfileActionError("duplicate JSON key")
        result[key] = value
    return result


def _reject_constant(value: str) -> Any:
    raise Group1ImtProfileActionError(f"non-finite JSON constant: {value}")
...
def _require_contract() -> None:
    ...
    exact = (
        ...
        (spec.repository_path, "Configuration_files/config_event_hazard_Group1.ini", "path"),
        (spec.byte_count, 1766, "byte count"),
    ...
    for observed, expected, label in exact:
        if type(observed) is not type(expected) or observed != expected:
            raise Group1ImtProfileActionError(f"trusted Group1 IMT {label} drifted")
    if _acquisition.acquire_event_hazard_group1_imt_profile is not _CANONICAL_ACQUIRER:
        raise Group1ImtProfileActionError("trusted Group1 IMT acquirer identity drifted")
...
def validate_request(body: object, *, expected_issue: int, execution_sha: str) -> dict[str, Any]:
    ...
    if type(expected_issue) is not int or expected_issue != CONTROL_ISSUE:
        raise Group1ImtProfileActionError("wrong runtime issue")
    if type(execution_sha) is not str or _SHA_RE.fullmatch(execution_sha) is None:
        raise Group1ImtProfileActionError("invalid execution SHA")
    if type(body) is not str or body.count(REQUEST_MARKER) != 1:
        raise Group1ImtProfileActionError("invalid Group1 IMT request marker")
    before, after = body.split(REQUEST_MARKER, 1)
    if before.strip() or not after.strip():
        raise Group1ImtProfileActionError("Group1 IMT request envelope is not canonical")
    try:
        request = json.loads(after.strip(), object_pairs_hook=_pairs, parse_constant=_reject_constant)
    except Group1ImtProfileActionError:
        raise
    except (json.JSONDecodeError, TypeError, ValueError) as exc:
        raise Group1ImtProfileActionError("invalid Group1 IMT request JSON") from exc
    if type(request) is not dict or set(request) != _REQUEST_FIELDS:
        raise Group1ImtProfileActionError("Group1 IMT request fields drifted")
    exact = (
    ...
    for field, expected in exact:
        ...
        if type(observed) is not type(expected) or observed != expected:
            raise Group1ImtProfileActionError(f"Group1 IMT request {field} drifted")
    ...
    if (
        ...
        raise Group1ImtProfileActionError("invalid requester identity")
    return request
...
def _validate_profile(profile: object) -> dict[str, Any]:
    ...
    if type(profile) is not dict or set(profile) != fields:
        raise Group1ImtProfileActionError("Group1 IMT profile fields drifted")
    spec = _bridge._root_spec(GROUP)
    exact = {
    ...
    for field, expected in exact.items():
        ...
        if type(observed) is not type(expected) or observed != expected:
            raise Group1ImtProfileActionError(f"Group1 IMT profile drifted at {field}")
...
def _validate_terminal_result(result: object, *, execution_sha: str) -> dict[str, Any]:
    ...
    if type(result) is not dict or set(result) != set(base) | {"status", "failure_class", "profile"}:
        raise Group1ImtProfileActionError("trusted Group1 IMT result fields drifted")
    for field, expected in base.items():
        observed = result.get(field)
        if type(observed) is not type(expected) or observed != expected:
            raise Group1ImtProfileActionError(f"trusted Group1 IMT result drifted at {field}")
    if result.get("status") == "pass":
        if result.get("failure_class") is not None:
            raise Group1ImtProfileActionError("Group1 IMT PASS cannot carry failure_class")
        _validate_profile(result.get("profile"))
        return result
    if result.get("status") == "blocked":
        if result.get("failure_class") != "profile_failure" or result.get("profile") is not None:
            raise Group1ImtProfileActionError("blocked Group1 IMT result is not safely bounded")
        return result
    raise Group1ImtProfileActionError("trusted Group1 IMT result has non-terminal status")
...
def _bounded_result_payload(after: str) -> str:
    ...
    try:
        ...
        raise Group1ImtProfileActionError("trusted Group1 IMT result is not valid UTF-8") from exc
    if payload_size > MAX_RESULT_UTF8_BYTES:
        raise Group1ImtProfileActionError("trusted Group1 IMT result exceeds publication limit")
    return payload
...
def _parse_trusted_terminal_result(body: object, *, execution_sha: str) -> bool:
    ...
    if body.count(RESULT_MARKER) != 1:
        raise Group1ImtProfileActionError("trusted Group1 IMT result marker is malformed")
    before, after = body.split(RESULT_MARKER, 1)
    if before.strip() or not after.strip():
        raise Group1ImtProfileActionError("trusted Group1 IMT result envelope is malformed")
    ...
    try:
        ...
        raise Group1ImtProfileActionError("trusted Group1 IMT result JSON is malformed") from exc
    if type(result) is not dict:
        raise Group1ImtProfileActionError("trusted Group1 IMT result is not an object")
    ...
    if (
        ...
        raise Group1ImtProfileActionError("trusted Group1 IMT historical SHA identity is inconsistent")
...
def has_terminal_imt_profile_result(
    *, repository: str, token: str, execution_sha: str, opener: Any | None = None, max_pages: int = 20
    ...
    try:
        ...
        raise Group1ImtProfileActionError("Group1 IMT result ledger is incomplete") from exc
    for comment in comments:
        if type(comment) is not dict:
            raise Group1ImtProfileActionError("Group1 IMT ledger contains a non-object comment")
...
def _run_profile(
    *, execution_sha: str, acquirer: Callable[[], dict[str, Any]]
    ...
    except (_acquisition.EventHazardDependencyAcquisitionError, Group1ImtProfileActionError):
