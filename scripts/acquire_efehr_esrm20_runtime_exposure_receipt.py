...
"""Trusted acquisition worker for the frozen ESRM20 Kosovo runtime exposure XML.
...
only; XML semantics, value wiring, model use, and publication remain separate.
"""
...
_CANONICAL_OPERATION_ID = "esrm20-v1-kosovo-runtime-exposure-xml-v1"
...
MAX_RUNTIME_EXPOSURE_BYTES = _CANONICAL_MAX_BYTES
...
def _require_canonical_aliases() -> None:
    aliases = (
        ...
        (MAX_RUNTIME_EXPOSURE_BYTES, _CANONICAL_MAX_BYTES, "maximum byte count"),
    )
    for observed, expected, label in aliases:
        if type(observed) is not type(expected) or observed != expected:
            raise EfehrAcquisitionError(f"frozen runtime exposure {label} drifted")
    ...
    for observed, expected, label in identities:
        if observed is not expected:
            raise EfehrAcquisitionError(
                f"frozen runtime exposure production {label} drifted"
            )
...
def _validate_core_receipt(receipt: object, *, file_url: str) -> dict[str, Any]:
    if type(receipt) is not dict:
        raise EfehrAcquisitionError("runtime exposure receipt is invalid")
    exact = (
        ("source_issue", _CANONICAL_SOURCE_ISSUE),
        ("dataset_id", _CANONICAL_DATASET_ID),
    ...
    if type(retrieved_at) is not str or not retrieved_at:
        raise EfehrAcquisitionError("runtime exposure retrieval time is invalid")
    ...
    if (
        ...
        raise EfehrAcquisitionError("runtime exposure byte count is invalid")
    if not _is_lower_sha256(receipt.get("sha256")):
        raise EfehrAcquisitionError("runtime exposure SHA-256 is invalid")
    ...
    for field in ("content_type", "etag"):
        ...
        if value is not None and type(value) is not str:
            raise EfehrAcquisitionError(f"runtime exposure {field} is invalid")
    return receipt


def _acquire_runtime_exposure_receipt(
...
def acquire_runtime_exposure_receipt() -> dict[str, Any]:
    """Fetch only the frozen Kosovo runtime exposure XML and hash its bytes."""

    _require_production_transport_identity()
    return _acquire_runtime_exposure_receipt(
