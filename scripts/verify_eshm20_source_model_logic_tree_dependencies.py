...
"""Verify receipted ESHM20 source-model logic-tree bytes before dependency parsing."""
...
try:
    ...
    from verify_eshm20_root_config_dependencies import (
        FROZEN_INVENTORY_PATHS,
        INVENTORY_RECEIPT_COMMENT_ID,
        PREFIX,
    )

SCHEMA_VERSION = "oc-eshm20-source-model-logic-tree-dependency-bridge-v1"
SOURCE_ISSUE = 281
RECEIPT_ISSUE = 361
...
ROOT_DISCOVERY_RESULT_COMMENT_ID = 5301726249
PARSER_ID = (
...
class VerifiedSourceModelLogicTreeError(ValueError):
    """Raised when source-model logic-tree bytes or dependencies violate frozen evidence."""


def _verify_payload_identity(payload: bytes) -> str:
    if type(payload) is not bytes:
        raise VerifiedSourceModelLogicTreeError("source-model logic-tree payload must be immutable bytes")
...
def extract_verified_source_model_dependencies(payload: bytes) -> dict[str, Any]:
    """Verify the exact receipted logic tree, then emit derived dependency metadata only."""
    ...
    if len(FROZEN_INVENTORY_PATHS) != 62:
        raise VerifiedSourceModelLogicTreeError("frozen ESHM20 inventory identity is invalid")
    if REPOSITORY_PATH not in FROZEN_INVENTORY_PATHS:
        raise VerifiedSourceModelLogicTreeError(
            "receipted source-model logic-tree path is absent from frozen inventory"
        )
    ...
    try:
        ...
        raise VerifiedSourceModelLogicTreeError(
            "verified source-model logic-tree dependency parse failed"
        ) from exc
    ...
    return {
        ...
        "root_discovery_result_comment_id": ROOT_DISCOVERY_RESULT_COMMENT_ID,
        "parser": PARSER_ID,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = extract_verified_source_model_dependencies(_read_regular_file(Path(args.input)))
