...
"""Execute a two-branch ESRM20 mixed-component numerical mechanics probe.

This is deliberately narrower than a hazard/reference run.  It evaluates one
provider-native RotD50 branch and one provider-native GEOMETRIC_MEAN branch in
the already reconstructed OpenQuake 3.14 runtime, using one explicit synthetic
context.  The result proves numerical execution mechanics and absence of the
known component-conversion wrapper/request on these two exact branches only.
It does not prove historical numerical agreement, component equivalence, or
hazard-to-vulnerability compatibility.
"""
...
class MixedComponentNumericProbeError(RuntimeError):
    """Raised when the bounded mixed-component numerical probe drifts."""
...
def validate_request(body: object, *, expected_issue: int, execution_sha: str) -> dict[str, Any]:
    ...
    if type(request) is not dict or set(request) != {
        "schema_version", "issue", "target_sha", "requester"
    }:
    ...
    if type(requester) is not str or not requester or len(requester) > 128:
...
def _selected_profile_record(profile: dict[str, Any], branch: dict[str, str]) -> dict[str, Any]:
    records = [
        record
        for record in profile.get("branches", [])
        if type(record) is dict
        and record.get("branch_set_id") == branch["branch_set_id"]
        and record.get("branch_id") == branch["branch_id"]
    ]
    if len(records) != 1:
        raise MixedComponentNumericProbeError("selected branch identity is not unique")
    record = records[0]
    if record.get("requested_gsim_token") != branch["requested_gsim_token"]:
        raise MixedComponentNumericProbeError("selected branch GSIM identity drifted")
    if record.get("argument_keys") != []:
        raise MixedComponentNumericProbeError("selected branch no longer has zero constructor arguments")
    return record


def _compute_probe_row(adapter: Any, model: object, branch: dict[str, str]) -> dict[str, Any]:
    try:
        import numpy as np
    ...
    runtime_keys = adapter.argument_keys_after_alias(model)
    if runtime_keys != []:
        raise MixedComponentNumericProbeError("selected branch gained post-alias constructor arguments")
    try:
    ...
    component = getattr(
        getattr(instance, "DEFINED_FOR_INTENSITY_MEASURE_COMPONENT", None), "name", None
    )
    ...
    ctx = np.zeros(
        1,
        dtype=[("mag", float), ("vs30", float), ("rrup", float), ("rhypo", float)],
    ).view(np.recarray)
    ...
    sig = np.zeros((1, 1), dtype=float)
    tau = np.zeros((1, 1), dtype=float)
    ...
    if pga_g <= 0.0 or sigma_ln < 0.0 or tau_ln < 0.0 or phi_ln < 0.0:
        raise MixedComponentNumericProbeError("selected branch numerical output is outside basic domain")

    distance_name = branch["distance_field"]
    distance_value = SYNTHETIC_CONTEXT[f"{distance_name}_km"]
    return {
        "branch_set_id": branch["branch_set_id"],
        "branch_id": branch["branch_id"],
        "requested_gsim_token": branch["requested_gsim_token"],
        ...
        "imt": "PGA",
        "input": {
            "mag": SYNTHETIC_CONTEXT["mag"],
            "vs30_m_per_s": SYNTHETIC_CONTEXT["vs30"],
...
def run_probe(*, execution_sha: str, image_digest: str) -> dict[str, Any]:
    ...
    return {
        ...
        "execution_container_image_digest": digest,
        "openquake_reference": {
            "repository": _gate.OPENQUAKE_REPOSITORY,
            "tag": _gate.OPENQUAKE_TAG,
            "commit": _esrm.OPENQUAKE_COMMIT,
            "version": _gate.OPENQUAKE_VERSION,
        },
        "gmm_identity": {
            "project_id": _esrm.PROJECT_ID,
            "project_path": _esrm.PROJECT_PATH,
            "commit_sha": _esrm.COMMIT_SHA,
            "repository_path": _esrm.REPOSITORY_PATH,
            "byte_count": _esrm.EXPECTED_BYTE_COUNT,
            "sha256": _esrm.EXPECTED_SHA256,
        },
        "synthetic_mechanics_probe": True,
        "synthetic_context": dict(SYNTHETIC_CONTEXT),
        "probes": rows,
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_probe(execution_sha=args.execution_sha, image_digest=image_digest)
