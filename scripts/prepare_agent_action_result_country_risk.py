...
The same bounded in-process bytes are then fed to the already-reviewed offline
schema profiler before being discarded; raw rows and numeric values never enter
the durable result.
"""
...
def prepare_completed_result(
    ...
    **kwargs: Any,
    ...
    """Run tree gate, fixed byte receipt and same-byte schema profile in order."""
