...
def validate_request(body: object, *, expected_issue: int, execution_sha: str) -> dict[str, Any]:
    ...
    if before.strip() or not after.strip():
        raise ScenarioWorkbookIdentityExecutionError("workbook identity request envelope is not canonical")
    try:
        request = json.loads(
        ...
        raise ScenarioWorkbookIdentityExecutionError("invalid workbook identity request JSON") from exc
    if type(request) is not dict or set(request) != _REQUEST_FIELDS:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity request fields drifted")
    if request["schema_version"] != REQUEST_SCHEMA_VERSION:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity request schema drifted")
    if type(request["issue"]) is not int or request["issue"] != SOURCE_ISSUE:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity request issue drifted")
    if request["target_sha"] != execution_sha:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity request target is not trusted main")
...
def _nonnegative_int(value: object, *, field: str, maximum: int | None = None) -> int:
    if type(value) is not int or isinstance(value, bool) or value < 0:
        raise ScenarioWorkbookIdentityExecutionError(f"{field} is not a nonnegative integer")
...
def validate_profile(value: object) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _PROFILE_FIELDS:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity profile fields drifted")
    exact = (
    ...
    for field, expected in exact:
        ...
        if type(observed) is not type(expected) or observed != expected:
            raise ScenarioWorkbookIdentityExecutionError(f"workbook identity profile drifted at {field}")
    ...
    if type(value["workbook_git_blob_sha1"]) is not str or _SHA1_RE.fullmatch(value["workbook_git_blob_sha1"]) is None:
    ...
    byte_count = _nonnegative_int(value["byte_count"], field="byte_count", maximum=profile.MAX_FILE_BYTES)
    if byte_count < 1:
        raise ScenarioWorkbookIdentityExecutionError("workbook byte count is empty")
    members = _nonnegative_int(value["zip_member_count"], field="zip_member_count", maximum=profile.MAX_ZIP_MEMBERS)
    if members < 1:
    ...
    worksheets = _nonnegative_int(value["worksheet_count"], field="worksheet_count", maximum=profile.MAX_WORKSHEETS)
    if worksheets < 1:
        raise ScenarioWorkbookIdentityExecutionError("workbook has no worksheets")
    _nonnegative_int(value["shared_string_count"], field="shared_string_count", maximum=profile.MAX_SHARED_STRINGS)
    _nonnegative_int(value["scanned_row_count"], field="scanned_row_count", maximum=profile.MAX_ROWS)
    _nonnegative_int(value["scanned_cell_count"], field="scanned_cell_count", maximum=profile.MAX_CELLS)
    target_cells = _nonnegative_int(value["target_event_id_exact_cell_count"], field="target_event_id_exact_cell_count")
    target_rows = _nonnegative_int(value["target_event_id_row_count"], field="target_event_id_row_count")
    if target_rows > target_cells:
        raise ScenarioWorkbookIdentityExecutionError("target row/cell counts disagree")

    cell_counts = _literal_counts(value["name_literal_cell_counts"], field="name_literal_cell_counts")
    row_counts = _literal_counts(value["name_literal_row_counts"], field="name_literal_row_counts")
    same_row = _literal_counts(
        value["target_same_row_name_literal_counts"],
        field="target_same_row_name_literal_counts",
    )
    for name in profile.NAME_LITERALS:
        if row_counts[name] > cell_counts[name]:
            raise ScenarioWorkbookIdentityExecutionError("name row/cell counts disagree")
        if same_row[name] > row_counts[name] or same_row[name] > target_rows:
            raise ScenarioWorkbookIdentityExecutionError("same-row binding counts disagree")
    binding = value["same_row_name_literal_binding"]
    if binding is not None and binding not in profile.NAME_LITERALS:
        raise ScenarioWorkbookIdentityExecutionError("same-row name binding is outside closed set")
    ...
    if len(positive) > 1 or binding != expected_binding:
        raise ScenarioWorkbookIdentityExecutionError("same-row name binding is contradictory")
    return value
...
def _load_result(body: object) -> dict[str, Any] | None:
    ...
    if body.count(RESULT_MARKER) != 1:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity result marker is malformed")
    before, after = body.split(RESULT_MARKER, 1)
    if before.strip() or not after.strip():
        raise ScenarioWorkbookIdentityExecutionError("workbook identity result envelope is malformed")
    try:
        value = json.loads(after.strip(), object_pairs_hook=_pairs, parse_constant=_reject_constant)
    except ScenarioWorkbookIdentityExecutionError:
        raise
    except (json.JSONDecodeError, TypeError, ValueError) as exc:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity result JSON is malformed") from exc
    if type(value) is not dict or set(value) != _RESULT_FIELDS:
        raise ScenarioWorkbookIdentityExecutionError("workbook identity result fields drifted")
    return value
...
def parse_terminal_result(body: object, *, execution_sha: str) -> bool:
    ...
    for field, expected in _base_result(execution_sha=execution_sha).items():
        observed = value[field]
        if type(observed) is not type(expected) or observed != expected:
            raise ScenarioWorkbookIdentityExecutionError(f"workbook identity result drifted at {field}")
    ...
    if value["status"] == "blocked":
        if value["failure_class"] != "workbook_identity_failure" or value["profile"] is not None:
            raise ScenarioWorkbookIdentityExecutionError("blocked result widened workbook evidence")
        return True
    if value["status"] == "duplicate":
        if value["failure_class"] is not None or value["profile"] is not None:
            raise ScenarioWorkbookIdentityExecutionError("duplicate result carries evidence")
        return True
    raise ScenarioWorkbookIdentityExecutionError("workbook identity result status is invalid")
...
def has_terminal_result(*, repository: str, token: str, execution_sha: str) -> bool:
    try:
        ...
        raise ScenarioWorkbookIdentityExecutionError("workbook result ledger is incomplete") from exc
    for comment in comments:
        if type(comment) is not dict:
            raise ScenarioWorkbookIdentityExecutionError("workbook result ledger contains non-object")
        ...
        if type(own_sha) is not str or _SHA1_RE.fullmatch(own_sha) is None:
            raise ScenarioWorkbookIdentityExecutionError("trusted workbook result SHA is invalid")
...
def execute_profile(*, repository: str, token: str, execution_sha: str) -> dict[str, Any]:
    if profile.acquire_and_profile_workbook_identity is not _PROFILE:
        raise ScenarioWorkbookIdentityExecutionError("trusted workbook profile authority drifted")
    if fetch_repository_comments is not _FETCH_COMMENTS:
        raise ScenarioWorkbookIdentityExecutionError("trusted workbook ledger authority drifted")
    ...
    if len(encoded) > MAX_RESULT_UTF8_BYTES:
        raise ScenarioWorkbookIdentityExecutionError("workbook result exceeds publication limit")
